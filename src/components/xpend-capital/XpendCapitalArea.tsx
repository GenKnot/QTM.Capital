import React from 'react';

const XpendCapitalArea = () => {
  return (
    <>
      <div className="xpend-capital-wrapper">
        <div className="divider"></div>

        <div className="container">
          <div className="row justify-content-center">
            <div className="col-12 col-lg-10">
              <div className="section-heading text-center">
                <h2 className="mb-4">我们的服务</h2>
                <div className="mission-statement">
                  <h3 className="text-primary mb-0">让每个中小企业<br />都有获得资本加持的权利</h3>
                </div>
              </div>
            </div>
          </div>

          <div className="divider-sm"></div>

          <div className="service-card-container">

            <div className="service-card">
              <div className="service-name-description">
                <h3>商业计划及商业孵化</h3>
                <p>帮助中小企业梳理商业模式，制定符合行业逻辑、投资人喜好、市场发展规律的商业计划书、可行性方案、财务规划、融资规划等。协助对接潜在合作方、意向客户及事业合伙人，帮助中小企业客户完成"蚂蚁变大象"的孵化过程。</p>
              </div>
              <div className="service-number">
                <h2>01</h2>
              </div>
            </div>

            <div className="service-card">
              <div className="service-name-description">
                <h3>私募股权及融资咨询</h3>
                <p>Xpend资本为处于初创期、成长期的中小企业提供专业、高效的咨询服务。通过提供专业、定制、整体的私募股权融资解决方案，以"外部合伙人"的身份支持客户制定着眼于长期可持续发展的资本市场战略。</p>
              </div>
              <div className="service-number">
                <h2>02</h2>
              </div>
            </div>

            <div className="service-card">
              <div className="service-name-description">
                <h3>上市辅导</h3>
                <p>针对部分体量适合的企业，进行有针对性的上市辅导。帮助中小企业在不改变原有盈利模式的基础上，进行团队、商业及产业模式的升级，以期待在资本市场获得更高的溢价空间。</p>
              </div>
              <div className="service-number">
                <h2>03</h2>
              </div>
            </div>

            <div className="service-card">
              <div className="service-name-description">
                <h3>直投基金</h3>
                <p>Xpend资本的直投美元基金可以参与到从基石到PRE-IPO的任何一轮投资过程中，与创业者共同成长。Xpend资本专注于投资中小企业，专注于智能产业、智能科技、智能医疗、智能消费和智能企业的"高信念"投资。</p>
              </div>
              <div className="service-number">
                <h2>04</h2>
              </div>
            </div>

            <div className="service-card">
              <div className="service-name-description">
                <h3>产业并购基金及并购辅导</h3>
                <p>Xpend资本积极寻找机会，将产业和金融投资者与具有高增长潜力的公司对接，为他们提供全面、深入的战略投资咨询和交易咨询服务，包括并购、分拆、关闭、重组、私有化和 De-SPAC。同时，针对中小企业所在行业特点，并对时下资本市场追捧的公司进行分析，寻找资本关注热点话题，进行并购企业的升级及包装。区别于传统并购市场"大鱼吃小鱼"的场景，Xpend资本的并购聚焦于帮助体量相同的企业进行抱团成长，共同扬长避短，享受资本红利。凭借无与伦比的行业网络和多样化、最新的产品组合，Xpend资本完成了多项具有里程碑意义的战略投资和并购交易。</p>
                <div className="service-action mt-3">
                  <a href="/industrial-mergers" className="btn btn-primary rounded-pill">
                    <span>产业并购</span>
                    <span>产业并购</span>
                  </a>
                </div>
              </div>
              <div className="service-number">
                <h2>05</h2>
              </div>
            </div>
          </div>
        </div>

        <div className="divider"></div>
      </div>
    </>
  );
};

export default XpendCapitalArea; 